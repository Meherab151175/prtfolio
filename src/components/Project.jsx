...
const Project = (project) => {
  const { name, img, description, technologies, livelink, github } = project;
  ...
  return (
    ...
    >
      <div
        className={`md:w-1/2 h-80 project-img bg-[url('${img}')] bg-cover`}
      ></div>
      ...
      <div className="relative md:w-1/2 h-auto p-6 pb-4 bg-secondary  project-cart">
        ...
        <div className=" translate-y-[2.25rem] z-30  flex items-center justify-center gap-10">
          ...
          <div
            className={`${
              server ? "visible" : "hidden"
            } bg-secondary rounded-full w-10 h-10 arrow-box flex justify-center items-center`}
          >
            <a
              href={server}
              target="_blank"
              rel="noreferrer"
              title="GitHub Server Repo"
            >
              <BsServer />
            </a>
          </div>
